...
public class DoctorService {
	...
	public Doctor register(Doctor doctor) throws InvalidInputException
	{
		...
		if(doctor.getAddress()==null)
			throw  new InvalidInputException(errorFiled);
		doctor.setId(UUID.randomUUID().toString());
		if(doctor.getSpeciality()==null)
			doctor.setSpeciality(Speciality.GENERAL_PHYSICIAN);
		Address doctorAddress = doctor.getAddress();
		Address address = new Address();
		address.setAddressLine1(doctorAddress.getAddressLine1());
		address.setAddressLine2(doctorAddress.getAddressLine2());
		address.setCity(doctorAddress.getCity());
		address.setState(doctorAddress.getState());
		address.setPostcode(doctorAddress.getPostcode());
		address.setId(UUID.randomUUID().toString());
		Address saveAddress = (Address) addressRepository.save(address);
		doctor.setAddress(saveAddress);
